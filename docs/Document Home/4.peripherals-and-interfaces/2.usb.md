...
The Qualcomm IM SDK includes the v4l2src plugin, which allows input from the USB camera in the selected format. The waylandsink plugin is responsible for rendering the video output on the Wayland display interface.
    ...
    :::
        * For 720p, run the following command:
        ```shell
        gst-launch-1.0 -e v4l2src device="/dev/video0" ! video/x-raw,format=YUY2,width=1280,height=720,framerate=10/1 ! waylandsink fullscreen=true        
        ```
        The output is as follows:
        ```
        ...
        * For 1080p, run the following command:
        ```shell
        gst-launch-1.0 -e v4l2src device="/dev/video0" ! video/x-raw,format=YUY2,width=1920,height=1080,framerate=5/1 ! waylandsink fullscreen=true       
        ```
    </TabItem>
...
To get the root privilege the following command.
sudo su
In case if you want to run from root privilege mode, simply type “exit” command.
